...
{
    public class RenderModel : MonoBehaviourPunCallbacks
    {
        public GameObject handPrefab;
        protected GameObject handInstance;
        protected Renderer[] handRenderers;
        protected Renderer[] skinnedHandRenderers;
        ...
        {
            if (handPrefab != null && photonView.IsMine)
            {
                Debug.Log("Instantiating from initialize hand in renderModel " + handPrefab.name);
                handInstance = PhotonNetwork.Instantiate(handPrefab.name, Vector3.zero, Quaternion.identity);
                ...
                skinnedHandRenderers = handInstance.GetComponentsInChildren<SkinnedMeshRenderer>();
        ...
        {
            if (handInstance != null && photonView.IsMine)
            {
        ...
        {
            Debug.Log("Set hand material: handRenderers.Length " + handRenderers.Length);
        ...
        public void SetHandVisibility(bool state, bool permanent = false)
        {
            ...
            {
                Debug.Log("Hand renderers are null");
                return;
            }

            else
            {
                Debug.Log("Hand renderers are not null and the length is: " + handRenderers.Length);
                Debug.Log("Hand renderers are not null and the length is: " + skinnedHandRenderers.Length);
            }
                

            for (int rendererIndex = 0; rendererIndex < handRenderers.Length; rendererIndex++)
            {
                Debug.Log("Disabling " + handRenderers[rendererIndex].name + " renderer");
        ...
        {
            //GameObject.FindGameObjectWithTag("ChessBoard").GetComponent<Renderer>().enabled = false;
            // for (int rendererIndex = 0; rendererIndex < handRenderers.Length; rendererIndex++)
            // {
            //     handRenderers[rendererIndex].enabled = false;
            // }
        }
