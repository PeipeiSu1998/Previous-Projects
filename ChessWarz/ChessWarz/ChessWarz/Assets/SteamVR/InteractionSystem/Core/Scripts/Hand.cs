    ...
    public class Hand : MonoBehaviourPunCallbacks
    {
        ...
        protected virtual IEnumerator Start()
        {
            ...
            while (true)
            {
                if (isPoseValid)
                {
                    Debug.Log("InitController from Start corutine");
                    InitController();
                    break;
                }
        ...
        protected virtual void Update()
        {
            if (photonView.IsMine)
            {
                ...
                {
                    attachedObject.
                    SendMessage("HandAttachedUpdate",
                                this,
        ...
        {
            if (photonView.IsMine && !handsInitialized)
            {
                if (spewDebugText)
                ...
                GameObject renderModelInstance = PhotonNetwork.Instantiate(renderModelPrefab.name, Vector3.zero, Quaternion.identity);
        ...
        public void SetRenderModel(GameObject prefab)
        {
            ...
            {
                Debug.Log("InitController from SetRenderModel");
                InitController();
            }
        }
