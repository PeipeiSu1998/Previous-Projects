		...
		private void HandAttachedUpdate( Hand hand )
		{
			...
			if ( nocked )
			{
				...
				if ( Vector3.Dot( nockToarrowHand, -nockTransform.forward ) > 0 )
				{
					...
					nockTransform.localPosition = new Vector3( 0f, 0f,Mathf.Clamp(
															   -distanceToarrowHand,
															   -maxPull, 0f ) );

					nockDistanceTravelled = -nockTransform.localPosition.z;

					arrowVelocity = Util.RemapNumber( nockDistanceTravelled, minPull, maxPull,
													  arrowMinVelocity, arrowMaxVelocity );
