...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Arrow : MonoBehaviourPunCallbacks
    {
            ...
            if (inFlight)
            {
                ...
                if (rbSpeed > 0.1f)
                {
                    if (photonView.IsMine)
                        collision.collider.gameObject.SendMessageUpwards("ApplyDamageFromOwnArrow",
                            SendMessageOptions.DontRequireReceiver);
                    else
                        collision.collider.gameObject.SendMessageUpwards("ApplyDamageFromOpponentsArrow",
                            SendMessageOptions.DontRequireReceiver);
        ...
        private void StickInTarget(Collision collision, bool bSkipRayCast)
        {
            ...
            {
                RaycastHit[] hitInfo;
                hitInfo = Physics.RaycastAll(prevHeadPosition - prevVelocity * Time.deltaTime, prevForward,
                    prevVelocity.magnitude * Time.deltaTime * 2.0f);
            ...
            transform.position = collision.contacts[0].point - transform.forward * (0.75f -
                (Util.RemapNumberClamped(prevVelocity.magnitude, 0f, 10f, 0.0f, 0.1f) + Random.Range(0.0f, 0.05f)));
        }
